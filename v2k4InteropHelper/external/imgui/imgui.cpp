...
void ImGui::CalcListClipping(int items_count, float items_height, int* out_items_display_start, int* out_items_display_end, int axis)
{
    ImGuiContext& g = *GImGui;
    ...
    if (axis == 1) {
        start = (int)((unclipped_rect.Min.y - pos.y) / items_height);
        end = (int)((unclipped_rect.Max.y - pos.y) / items_height);
    }
    else {
        start = (int)((unclipped_rect.Min.x - pos.x) / items_height);
        end = (int)((unclipped_rect.Max.x - pos.x) / items_height);
    }
    ...
    if (axis == ImGuiAxis_Y) {
        ...
        if (g.NavMoveRequest && g.NavMoveClipDir == ImGuiDir_Left)
            start--;
        if (g.NavMoveRequest && g.NavMoveClipDir == ImGuiDir_Right)
            end++;
    }
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height, int axis)
{
    ...
    if (axis == ImGuiAxis_Y) {
        ...
        float off_y = pos_y - window->DC.CursorPos.x;
        window->DC.CursorPos.x = pos_y;
        window->DC.CursorMaxPos.x = ImMax(window->DC.CursorMaxPos.x, pos_y);
        window->DC.CursorPosPrevLine.x = window->DC.CursorPos.x - line_height;  // Setting those fields so that SetScrollHereY() can properly function after the end of our clipper usage.
        window->DC.PrevLineSize.x = (line_height - g.Style.ItemSpacing.x);      // If we end up needing more accurate data (to e.g. use SameLine) we may as well make the clipper have a fourth step to let user process and display the last item in their list.
    }
...
void ImGuiListClipper::Begin(int count, float items_height, int axis)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    if (axis == 1) {
        StartPos = window->DC.CursorPos.y;
    }
    else {
        StartPos = window->DC.CursorPos.x;
    }
    ...
    if (ItemsHeight > 0.0f)
    {
        ImGui::CalcListClipping(ItemsCount, ItemsHeight, &DisplayStart, &DisplayEnd, axis); // calculate how many to clip/display
        if (DisplayStart > 0)
            SetCursorPosYAndSetupDummyPrevLine(StartPos + DisplayStart * ItemsHeight, ItemsHeight, axis); // advance cursor
        StepNo = 2;
    }
}
...
void ImGuiListClipper::End()
{
    ...
    if (ItemsCount < INT_MAX)
        SetCursorPosYAndSetupDummyPrevLine(StartPos + ItemsCount * ItemsHeight, ItemsHeight, ax); // advance cursor
    ItemsCount = -1;
    StepNo = 3;
}
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 1) // Step 1: the clipper infer height from first element, calculate the actual range of elements to display, and position the cursor before the first element.
    {
        ...
        {
            if (ax == ImGuiAxis_Y) {
                items_height = window->DC.CursorPos.y - StartPos;
            }
            else {
                items_height = window->DC.CursorPos.x - StartPos;
            }
            
            IM_ASSERT(items_height > 0.0f);         // If this triggers, it means Item 0 hasn't moved the cursor vertically
        }
        Begin(ItemsCount - 1, items_height, ax);
        DisplayStart++;
        DisplayEnd++;
        StepNo = 3;
        return true;
