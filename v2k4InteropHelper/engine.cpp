...
#include "misc.h"
    ...
    void Engine::LogBuildInfo() {
        ...
        if (!pBuildInfo) {
            ...
            OutputDebugStringA("Engine.BuildInfo.dll not found");
            return;
        }
    ...
    void Engine::Setup() {
        ...
        if (pCurrentScreenID == 0) {
            ...
            OutputDebugString("Engine::Setup::pCurrentScreenID==0::Unable to Continue");
            return;
        }
    ...
    ScriptService* Engine::GetScriptService() {
        ...
        uintptr_t pScriptFunctions = g_ctx_proc.getAddr("AOB_pScriptFunctions", true, 61);
        logger.Write(LOG_DEBUG, "pScriptFunctions:  0x%08llX", pScriptFunctions);
        auto script_functions = reinterpret_cast<ScriptFunctions*>(pScriptFunctions);
        ...
        auto script_service = reinterpret_cast<ScriptService*>(pScript);
    ...
    {
        if (isInCM()) {
            OutputDebugString("Engine::RunFIFAScript::");
