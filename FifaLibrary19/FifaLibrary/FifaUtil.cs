...
namespace FifaLibrary
{
	public class FifaUtil
	{
		...
		private static uint EAHash(byte[] str, int length)
		{
			...
			return (uint)((int)num ^ int.MinValue);
		}
		...
		private static uint ELFHash(byte[] str, int length)
		{
			...
			{
				num = (num << 4) + str[i];
				if ((num2 = (uint)((int)num & -268435456)) != 0)
				{
					num ^= num2 >> 24;
				}
				num &= ~num2;
		...
		private static uint MurmurHash2(byte[] str, int length)
		{
			...
			if (length == 3)
			{
				num = (uint)((int)num ^ (str[num2 + 2] << 16));
			}
			if (length >= 2)
			{
				num = (uint)((int)num ^ (str[num2 + 1] << 8));
			}
			num ^= str[num2];
			num *= 1540483477;
			num ^= num >> 13;
		...
		{
			DateTime d = new DateTime(1582, 10, 14, 0, 0, 0);
			return (date - d).Days;
		}
