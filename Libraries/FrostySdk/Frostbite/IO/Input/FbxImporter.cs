...
namespace FrostySdk.Frostbite.IO.Input
{
	...
	public class FBXImporter
	{
		...
		public void ImportFBX(string filename, MeshSet inMeshSet, EbxAsset asset, EbxAssetEntry entry, MeshImportSettings inSettings)
		{
			...
			//entry.ClearLinkedAssets();
			//resEntry.ClearLinkedAssets();
		...
		*/
		private void ProcessLod(List<FbxNode> nodes, int lodIndex)
		{
			...
			fbxNode
				...
				list8.Add(memoryStream.ToArray());
				list9.Add(list10);
				num2 += (uint)(int)memoryStream.Length;
				memoryStream.Dispose();
				list7.Add(fbxNode);
			...
			for (int j = 0; j < list3.Count; j++)
			{
				...
				if (j == 0)
				{
					...
					list8.Add(memoryStream2.ToArray());
					list9.Add(list11);
					num2 += (uint)(int)memoryStream2.Length;
					memoryStream2.Dispose();
				}
			...
			for (int k = 0; k < list4.Count; k++)
			{
				...
				if (k == 0)
				{
					...
					list8.Add(memoryStream3.ToArray());
					list9.Add(list12);
					num2 += (uint)(int)memoryStream3.Length;
					memoryStream3.Dispose();
				}
			...
			using NativeWriter nativeWriter = new NativeWriter(new MemoryStream());
			foreach (byte[] item2 in list8)
			{
				nativeWriter.Write(item2);
			}
			nativeWriter.WritePadding(16);
			meshSetLod.VertexBufferSize = (uint)nativeWriter.BaseStream.Position;
		...
		private unsafe void ProcessSection(FbxNode[] sectionNodes, MeshSetLod meshLod, int sectionIndex, MemoryStream verticesBuffer, List<uint> indicesBuffer, uint vertexOffset, ref uint startIndex)
		{
			...
			foreach (FbxNode fbxNode2 in array5)
			{
				...
				//{
				//	throw new Exception("Missing UV");
				...
				for (int l = 0; l < fbxMesh2.PolygonCount; l++)
				{
					for (int num38 = 0; num38 < 3; num38++)
					{
						...
						for (int num13 = 0; num13 < elements.Length; num13++)
						{
							...
							//throw new FBXImportUnimplementDataTypeException(element.Usage);
						}
						int num23 = -1;
							...
							if (num28 >= num27 && num28 < num27 + stream.VertexStride)
							{
								switch (element2.Usage)
								{
									case VertexElementUsage.Pos:
										...
										{
											nativeWriter.Write((ushort)HalfUtils.Pack(val11.X));
											nativeWriter.Write((ushort)HalfUtils.Pack(val11.Y));
											nativeWriter.Write((ushort)HalfUtils.Pack(val11.Z));
											if (element2.Format == VertexElementFormat.Half4)
											{
												nativeWriter.Write((ushort)HalfUtils.Pack(1f));
											}
										}
										break;
									case VertexElementUsage.BinormalSign:
										...
										{
											nativeWriter.Write((ushort) HalfUtils.Pack((Vector3.Dot(Vector3.Cross(val12, val13), val2) < 0f) ? 1f : (-1f)));
										}
										else
										{
									...
									case VertexElementUsage.BoneIndices:
										if (value3 == null)
										{
											...
											//throw new FBXImportMissingBonesException();
										}
										if (element2.Format == VertexElementFormat.UShort4)
										{
											nativeWriter.Write((ushort)value3[4]);
											nativeWriter.Write((ushort)value3[5]);
											nativeWriter.Write((ushort)value3[6]);
											nativeWriter.Write((ushort)value3[7]);
										}
										else
										{
									...
									case VertexElementUsage.BoneIndices2:
										...
										if (element2.Format == VertexElementFormat.UShort4)
										{
											nativeWriter.Write((ushort)value3[0]);
											nativeWriter.Write((ushort)value3[1]);
											nativeWriter.Write((ushort)value3[2]);
											nativeWriter.Write((ushort)value3[3]);
										}
										else
										{
									...
									case VertexElementUsage.BoneWeights2:
										...
										break;
									case VertexElementUsage.Binormal:
										nativeWriter.Write((ushort)HalfUtils.Pack(val2.X));
										nativeWriter.Write((ushort)HalfUtils.Pack(val2.Y));
										nativeWriter.Write((ushort)HalfUtils.Pack(val2.Z));
										nativeWriter.Write((ushort)HalfUtils.Pack(1f));
										break;
									case VertexElementUsage.Normal:
										nativeWriter.Write((ushort)HalfUtils.Pack(val12.X));
										nativeWriter.Write((ushort)HalfUtils.Pack(val12.Y));
										nativeWriter.Write((ushort)HalfUtils.Pack(val12.Z));
										nativeWriter.Write((ushort)HalfUtils.Pack(1f));
										break;
									case VertexElementUsage.Tangent:
										nativeWriter.Write((ushort)HalfUtils.Pack(val13.X));
										nativeWriter.Write((ushort)HalfUtils.Pack(val13.Y));
										nativeWriter.Write((ushort)HalfUtils.Pack(val13.Z));
										nativeWriter.Write((ushort)HalfUtils.Pack(1f));
										break;
									case VertexElementUsage.TangentSpace:
										...
										break;
									case VertexElementUsage.RadiosityTexCoord:
										nativeWriter.Write((ushort)HalfUtils.Pack(0f));
										nativeWriter.Write((ushort)HalfUtils.Pack(0f));
										break;
									case VertexElementUsage.Color0:
									...
									case VertexElementUsage.Delta:
										if (item3.HasValue("Delta"))
										{
											...
											nativeWriter.Write((ushort)HalfUtils.Pack((float)(int)value10.R * 255f));
											nativeWriter.Write((ushort)HalfUtils.Pack((float)(int)value10.G * 255f));
											nativeWriter.Write((ushort)HalfUtils.Pack((float)(int)value10.B * 255f));
											nativeWriter.Write((ushort)HalfUtils.Pack((float)(int)value10.A * 255f));
										}
										else
										{
											nativeWriter.Write((ushort)HalfUtils.Pack(0f));
									...
									default:
										if ((int)element2.Usage >= 33 && (int)element2.Usage <= 40)
										{
											...
											if (item3.HasValue(name2))
											{
												...
												nativeWriter.Write((ushort)HalfUtils.Pack(value5.X));
												nativeWriter.Write((ushort)HalfUtils.Pack(1f - value5.Y));
											}
											else
											{
												nativeWriter.Write((ushort)0);
										...
										//throw new FBXImportUnimplementDataTypeException(element2.Usage);
									case VertexElementUsage.Unknown:
										break;
								}
							}
