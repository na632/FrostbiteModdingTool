...
namespace FrostySdk.Frostbite.IO.Input.FT
{
	public class FBXImporter2
	{
		...
		public float FlipZ { get; set; } = 1f;


		public FBXImporter2(AssetManager assetManager)
		{
		...
		public void ImportFBX(string filename, MeshSet inMeshSet, EbxAsset asset, EbxAssetEntry entry, MeshImportSettings inSettings)
		{
			...
			if (inMeshSet == null)
			{
				throw new ArgumentNullException("inMeshSet");
			}
			if (asset == null)
			{
			...
			using (FbxManager fbxManager = new FbxManager())
			{
				...
				foreach (FbxNode child in fbxScene.RootNode.Children)
				{
					...
					int colonIndex = text.IndexOf(':', StringComparison.OrdinalIgnoreCase);
					if (colonIndex != -1)
					{
				...
				if (lodsCount == 0)
				{
				throw new Exception("No LODs!");
				}
				if (lodsCount < meshSet.Lods.Count)
				{
					throw new Exception("Missing LODs!");

					//Log.Warning("Mesh only has {ImportedLodCount} LOD(s), and should have {OriginalLodCount}. The missing LODs will be auto-generated from the lowest-quality LOD.", lodsCount, meshSet.Lods.Count);
		...
		private void ProcessLod(List<FbxNode> nodes, int lodIndex)
		{
			...
			for (int i = 0; i < list8.Count; i++)
			{
				...
				ProcessSection(new FbxNode[1] { fbxNode }, meshSetLod, sectionIndex, memoryStream, list2, num2, ref startIndex);
			...
			foreach (List<uint> item4 in list12)
			{
				foreach (uint item3 in item4)
				{
					...
					{
						nativeWriter.WriteUInt16LittleEndian((ushort)item3);
					}
				}
			}
		...
		private unsafe void ProcessSection(FbxNode[] sectionNodes, MeshSetLod meshLod, int sectionIndex, MemoryStream verticesBuffer, List<uint> indicesBuffer, uint vertexOffset, ref uint startIndex)
		{
			...
			foreach (FbxNode fbxNode2 in array5)
			{
				...
				for (int l = 0; l < fbxMesh2.PolygonCount; l++)
				{
					for (int num38 = 0; num38 < 3; num38++)
					{
						...
						for (int num13 = 0; num13 < elements.Length; num13++)
						{
							...
							throw new Exception("FBXImportUnimplementDataTypeException");
						}
						int num23 = -1;
				...
				FileWriter nativeWriter = new FileWriter(verticesBuffer);
