    ...
    public class MeshSet2
    {
		...
		public TangentSpaceCompressionType TangentSpaceCompressionType
		{
			...
			set
			{
				...
				foreach (MeshSetLod2 lod in Lods)
				{
		...
		public List<MeshSetLod2> Lods { get; } = new List<MeshSetLod2>();
		...
		public MeshSet2(Stream stream, AssetManager am)
		{
			if (stream == null)
			{
			...
			{
				Lods.Add(new MeshSetLod2(nativeReader, am));
			}
			int sectionIndex = 0;
			foreach (MeshSetLod2 lod4 in Lods)
			{
				for (int m = 0; m < lod4.Sections.Count; m++)
				{
					lod4.Sections[m] = new MeshSetSection2(nativeReader, am, sectionIndex++);
				}
			}
			nativeReader.Pad(16);
			...
			nativeReader.Pad(16);
			foreach (MeshSetLod2 lod3 in Lods)
			{
			...
			nativeReader.Pad(16);
			foreach (MeshSetLod2 lod2 in Lods)
			{
			...
			nativeReader.Pad(16);
			foreach (MeshSetLod2 lod5 in Lods)
			{
		...
		private void Process(NativeWriter writer, MeshContainer meshContainer)
		{
			...
			foreach (MeshSetLod2 lod3 in Lods)
			{
				...
				foreach (MeshSetSection2 section3 in lod3.Sections)
				{
			...
			foreach (MeshSetLod2 lod5 in Lods)
			{
				foreach (MeshSetSection2 section2 in lod5.Sections)
				{
			...
			writer.WritePadding(16);
			foreach (MeshSetLod2 lod6 in Lods)
			{
