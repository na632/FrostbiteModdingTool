...
{
    public class MeshSetToFbxExport2
	{
		...
		public void Export(AssetManager assetManager, dynamic meshAsset, string filename, string fbxVersion, string units, bool flattenHierarchy, string skeleton, string fileType, params MeshSet2[] meshSets)
		{
			...
			FbxScene fbxScene = new FbxScene(fbxManager, "")
			{
				...
				{
					Title = "FBX Exporter",
					Subject = "Export FBX meshes",
					OriginalApplicationVendor = "",
					OriginalApplicationName = "",
					OriginalApplicationVersion = "",
					LastSavedApplicationVendor = "",
					LastSavedApplicationName = "",
			...
			totalExportCount++;
			MeshSet2[] array = meshSets;
			for (int k = 0; k < array.Length; k++)
			{
				foreach (MeshSetLod2 lod in array[k].Lods)
				{
					foreach (MeshSetSection2 section in lod.Sections)
					{
			...
			currentProgress++;
			array = meshSets;
			foreach (MeshSet2 meshSet in array)
			{
		...
		private void FBXCreateMesh(AssetManager assetManager, FbxScene scene, MeshSetLod2 lod, List<FbxNode> boneNodes)
		{
			if (assetManager == null)
			{
				throw new ArgumentNullException("assetManager");
		...
		private void FBXCreateBindPose(FbxScene scene, MeshSetSection2 section, FbxNode actor)
		{
		...
		private void FBXCreateSkin(FbxScene scene, MeshSetSection2 section, FbxNode actor, List<FbxNode> boneNodes, List<ushort> boneList, MeshType meshType, NativeReader reader)
		{
			FbxMesh fbxMesh = new FbxMesh(actor.GetNodeAttribute(FbxNodeAttribute.EType.eMesh));
		...
		private unsafe FbxNode FBXExportSubObject(FbxScene scene, MeshSetSection2 section, long indicesOffset, int indexSize, NativeReader reader)
		{
