    ...
    public static class ImageEngine
    {
        ...
        internal static void TestDDSMipSize(List<MipMap> newMips, ImageFormats.ImageEngineFormatDetails destFormatDetails, int width, int height, out double fixXScale, out double fixYScale, MipHandling mipChoice)
        {
            ...
            if (destFormatDetails.IsBlockCompressed)
            {
                ...
                if (aspect > 1)
                {
                    newWidth = RoundToNearestPowerOfTwo(width);
                    var tempScale = newWidth / width;
                    newHeight = RoundToNearestPowerOfTwo((int)(height * tempScale));
                }
                else
                {
        ...
        {
            var baseBMP = CreateWriteableBitmap(mipMap.Pixels, mipMap.Width, mipMap.Height);
            baseBMP.Freeze();
            ...
            var bmp = CreateWriteableBitmap(mipMap.Pixels, mipMap.Width, mipMap.Height);
