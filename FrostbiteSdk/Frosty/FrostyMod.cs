...
namespace FrostySdk
{
	public class FrostyMod
	{
		...
		public FrostyMod(string inFilename)
		{
			...
			using (var fs = new FileStream(path, FileMode.Open))
			{
                using (FrostyModReader frostyModReader = new FrostyModReader(fs))
                    //FrostyModReader frostyModReader = new FrostyModReader(fs);
				{
		...
		public FrostyMod(Stream stream)
		{
			using (FrostyModReader frostyModReader = new FrostyModReader(stream))
			{
				if (frostyModReader.IsValid)
				{
					bNewFormat = true;
		...
		public byte[] GetResourceData(BaseModResource resource, Stream stream)
		{
			stream.Position = 0;
			//using (FrostyModReader frostyModReader = new FrostyModReader(stream))
			FrostyModReader frostyModReader = new FrostyModReader(stream);
			{
				return frostyModReader.GetResourceData(resource);
			}
		}
