...
namespace paulv2k4ModdingExecuter
{
    public class FrostyModExecutor
	{
		...
		private Dictionary<string, List<string>> addedBundles = new Dictionary<string, List<string>>();
		...
		public async Task<int> Run(FileSystem inFs, ILogger inLogger, string rootPath, string additionalArgs, params string[] modPaths)
		{
            ...
            var process = Process.GetProcesses()
                .FirstOrDefault(x =>
                x.ProcessName.Equals(ProfilesLibrary.ProfileName, StringComparison.OrdinalIgnoreCase));
            if (process != null)
                return -1;
			...
			//{
				...
				if (rebuildModDataFolder)
				{
					...
					foreach (string str in array2)
					{
						...
						FrostyMod frostyMod2 = new FrostyMod(str);
						...
						{
							DbObject dbObject2 = null;
							using (DbReader dbReader2 = new DbReader(new FileStream(str, FileMode.Open, FileAccess.Read), null))
							{
							...
							foreach (DbObject item2 in value)
							{
								...
								else if (value6 == "ebx")
								{
									...
									if (item2.HasValue("archiveIndex"))
									{
										...
										array3 = GetResourceData(str, item2.GetValue("archiveIndex", 0), item2.GetValue("archiveOffset", 0L), (int)ebxAssetEntry4.Size);
									}
									else
									{
								...
								else if (value6 == "res")
								{
									...
									if (item2.HasValue("archiveIndex"))
									{
										...
										array4 = GetResourceData(str, item2.GetValue("archiveIndex", 0), item2.GetValue("archiveOffset", 0L), (int)resAssetEntry5.Size);
									}
									else
									{
								...
								else if (value6 == "chunk")
								{
									...
									if (item2.HasValue("archiveIndex"))
									{
										...
										array5 = GetResourceData(str, item2.GetValue("archiveIndex", 0), item2.GetValue("archiveOffset", 0L), (int)chunkAssetEntry5.Size);
									}
									else
									{
