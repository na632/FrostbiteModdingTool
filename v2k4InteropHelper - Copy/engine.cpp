...
void Engine::SetupMainLua() {
    ...
    //uintptr_t addr_max = g_ctx_proc.getAddr("AOB_LUAEngineFuncReg", true, 3);
    //uintptr_t addr_item = g_ctx_proc.getAddr("AOB_LUAEngineFuncReg", true, 10);

    //const __int32 n_of_funcs = *reinterpret_cast<__int32*>(addr_max);

    //const __int32 sizeOfStruct = 0x18;
    //for (__int32 i = 0; i < n_of_funcs; i++) {
    //    __int32 offset = i * sizeOfStruct;
    //    uintptr_t addr = addr_item + offset;
    //    LuaCMEngineFuncReg* currentF = reinterpret_cast<LuaCMEngineFuncReg*>(addr);
