...
#ifdef _DEBUG
    logger.Write(LOG_DEBUG, "TEST LOG_DEBUG");
    logger.Write(LOG_INFO, "TEST LOG_INFO");
    logger.Write(LOG_WARN, "TEST LOG_WARN");
    logger.Write(LOG_ERROR, "TEST LOG_ERROR");
    logger.Write(LOG_FATAL, "TEST LOG_FATAL");
#endif
...
{
    const std::string logPath = "G:\\Work\\FIFA Modding\\FIFAModdingUI\\v2k4InteropHelper\\x64\\Debug\\insideDLL";
    if (!fs::is_directory(logPath) || !fs::exists(logPath)) {
        fs::create_directory(logPath);
    }
